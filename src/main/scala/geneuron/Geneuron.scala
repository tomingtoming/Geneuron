...
import geneuron.creature.{Creature, Neuron, Physics, Status}
...
class Geneuron(gamename: String) extends BasicGame(gamename) {
  ...
  def init(gc: GameContainer): Unit = {
    ...
    creatures += new Creature(
      new Physics(Random.nextFloat(), Random.nextFloat(), Random.nextFloat(), Random.nextFloat(), Random.nextFloat(), Random.nextFloat()),
      new Neuron(Array.empty, Array.empty),
      new Status(Random.nextFloat(), Random.nextFloat(), Random.nextFloat(), Random.nextFloat(), Random.nextFloat())
    )
  }
  def update(gc: GameContainer, i: Int): Unit = creatures.foreach { creature =>
